...
const getAll = async () => { // esta funcion asicronica deberia comunicarse con la base de datos para obtener todos los productos
    try {
        const response = await fetch(`https://backend-elegance-mode.onrender.com/api/productos`, { mode: 'no-cors' }) // esta peticion es para obtener todos los productos de la base de datos a traves del render
...
const agregarcarrito = (e) => { // esta funcion es para agregar los productos al carrito
    ...
    fetch(`https://backend-elegance-mode.onrender.com/api/productos/carrito/${idproducto}`,{ mode: 'no-cors' }) // esta peticion es para agregar el producto al carrito a la base de datos a traves del render
        .then(res => res.json())
        .then(json => {
